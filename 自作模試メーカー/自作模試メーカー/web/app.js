...
function getHistoryRowsHtml(h) {
  const judgeLabel = (judgeRaw) => {
    const judge = String(judgeRaw || "").trim();
    if (judge === "○") return "正解";
    if (judge === "×") return "不正解";
    if (judge === "△") return "一部正解";
    if (judge === "自己判定") return "自己判定";
    return judge || "-";
  };
  return (h.rows || [])
    .map(
      (r) =>
        `第${r.no}問 ${r.type} | 問題: ${escapeHtml(r.question)} | 回答: ${escapeHtml(r.userAnswer)} | 正答: ${escapeHtml(r.correctAnswer)} | 判定: ${judgeLabel(r.judge)}`
    )
    .join("<br />");
}
...
function renderHistory() {
  ...
  els.historyList.innerHTML = filtered
    .map((h) => {
      const detail = getHistoryRowsHtml(h);
...
function renderHomeDashboard() {
  if (!els.homeRecentHistoryList) return;
  const questionCount = state.questions.length;
  const genreCount = countAllGenres();
